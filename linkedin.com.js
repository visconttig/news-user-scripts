...
(function () {
  ...
  const shouldHidePeopleBlock = (text) => {
    ...
    const isPeopleBlock =
      lowered.includes("you may know") || lowered.includes("to follow");

    const isPersonalizedByActivity = lowered.includes("activity");

    const isTargetingCurrentCity = new RegExp(
      `\\b${currentCity.toLowerCase()}\\b`
    ).test(lowered);

    return (
      isPeopleBlock && !isPersonalizedByActivity && !isTargetingCurrentCity
    );
  };
  ...
  const selectorMap = {
    feedPost:
      ".feed-shared-update-v2__control-menu-container.display-flex.flex-column.flex-grow-1.full-height",
  };
